...
class ProductCreate extends Component {
					...
					<div className="col-md-4">
						<div className="images_box">
							<ImageUploader
								...
								withPreview={true}
								singleImage={true}
								onChange={(picture, base64) => this.productImages(picture, base64, name)}
			...
			<form onSubmit={handleSubmit(this.dataSubmit)}>
				<div className="row">
					<div className="col-md-12">
						...
						<div className="row">
							{this.renderImages('thread_view', 'General view')}

							{this.renderImages('front_view', 'Front view')}

							{this.renderImages('right_view', 'Right view')}
						
							{this.renderImages('back_view', 'Back view')}

							{this.renderImages('left_view', 'Left view')}
						</div>
	...
	_addVariants() {
		...
		return(
			<Modal visible={this.state.visible} className="modal" width="1000" height="600" effect="fadeInUp" onClickAway={() => this.setState({visible : false})}>
	...
	_VariantsFiled() {
		...
		return(
			<>
				...
				<div className="row">
					{this.renderImages('thread_view_variant', 'General view')}
					
					{this.renderImages('front_view_variant', 'Front view')}

					{this.renderImages('right_view_variant', 'Right view')}

					{this.renderImages('back_view_variant', 'Back view')}

					{this.renderImages('left_view_variant', 'Left view')}
				</div>
			</>
		);
	}
...
const validate = (values) => {
	...
	//General View
	if (!values.thread_view) {
		errors.thread_view = 'General View Required'
	}
	//Front View
	if (!values.front_view) {
		errors.front_view = 'Front View Required'
	}
	//Right View
	if (!values.right_view) {
		errors.right_view = 'Right View Required'
	}
	//Back View
	if (!values.back_view) {
		errors.back_view = 'Back View Required'
	}
	//Left View
	if (!values.left_view) {
		errors.left_view = 'Left View Required'
	}
